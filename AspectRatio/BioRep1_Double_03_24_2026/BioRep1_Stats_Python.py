...
def fig_posthoc(R):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(10, max(3, n*0.32+1.2)),
    ...
    short = [f"{g1[i].split('|')[-1].strip()} vs {g2[i].split('|')[-1].strip()}"
             for i in range(n)]
    ax.set_yticks(y_pos)
    ...
    t = ax2.table(cellText=tbl_rows, colLabels=col_labels,
                  colWidths=[0.25,0.25,0.18,0.20,0.12],
                  cellLoc='center', loc='center')
...
def fig_effects(R):
    ...
    for thr, lbl, ls in [(0.01,'Small',':'),(0.06,'Medium','--'),(0.14,'Large','-')]:
